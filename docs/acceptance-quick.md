...
> 为什么还有手动验收：桥接层与真实 Electron 行为（`bridge.ts`、权限模型是否可用）
> 无法在 `node --test` 里覆盖。项目按用户决策**不跑** `@vscode/test-electron` e2e。
> 自动化能覆盖的部分是 215 项测试（214 通过 + 1 个 `--expose-gc` 严格用例按设计 skip）。
