...
（`packages/host/test/isolation.spec.ts`，数量以 `pnpm run verify` 输出为准）。
此外已在**本机 Electron-as-Node 模式**预验证 `--permission` 可用（VSCode 1.118.1 /
Electron 39.8.8 / Node 22.22.1），但下面这部分仍必须用**真实 Extension Host(F5)** 验收。
...
| 2 | `VSCordis: 显示运行时状态` | 插件表里 `isolated-hello` 为 `active`，`provides=-`（该示例本身不提供服务；隔离插件可通过 `ctx.async.useService` 消费远程服务，见 ADR-0019） |
| 3 | `VSCordis: 运行插件命令…` → `isolated-hello.greet` | 弹出「来自隔离子进程的问候：world」 |
| 4 | 打开任务管理器，看扩展宿主拉起的子进程 | 真实 VSCode 下进程名通常是 `Code.exe`（`ELECTRON_RUN_AS_NODE=1` 的 Node 模式；纯 Node 调试时可能是 `node.exe`） |
...
说明当前 VSCode/Electron 组合不支持 `--permission`（Electron-as-Node 预验证通过，但真实
Extension Host 仍以本步骤结果为准）。临时逃生开关：
...
请把 VSCode/Electron 版本、报错原文与开关前后结果写进反馈；ADR-0013 缺口 1 已记录
Electron-as-Node 预验证证据与"仍待 F5"的范围声明。
